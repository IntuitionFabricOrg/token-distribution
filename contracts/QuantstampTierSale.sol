 ...
 */
contract QuantstampTierSale is Pausable {
    ...
    mapping(address => uint256) public prepaid;     // prepaid contribution
    ...
    function QuantstampSale(
        ...
        require(minimumContributionInWei > 0);
    ...
    function registerMany(uint numCustomers, address[] customers, uint[] _cap1,
            uint[] _cap2, uint[] _cap3, uint[] _cap4, uint[] _prepaid)
        public
        onlyOwner
    {
        require(numCustomers > 0);
        for (uint i = 0; i < numCustomers; i++) {
            register(customers[i], _cap1[i], _cap1[i], _cap1[i], _cap4[i])
        }
    }

    /**
     * Registers the customer. Only registered customers can buy QSP tokens.
     */
    function register(address customer, uint _cap1, uint _cap2, uint _cap3, uint _cap4)
        public
        onlyOwner
    {
        require(customer != address(0));
        require(!hasPreviouslyRegistered(customer));
        require(cap1 + cap2 + cap3 + cap4 >= minContribution);

        cap1[customer] = cap1;
        cap2[customer] = cap2;
        cap3[customer] = cap3;
        cap4[customer] = cap4;
        registry[customer] = true;
    }

    function deactivate(address customer)
        public
        onlyOwner
    {
        require(hasPreviouslyRegistered(customer));
        registry[customer] = false;
    }

    function reactivate(address customer)
        public
        onlyOwner
    {
        require(hasPreviouslyRegistered(customer));
    ...
    function hasPreviouslyRegistered(address customer) bool
        internal
        onlyOwner
    {
        // if caps for this customer exist, then the customer has previously been registered
        return (cap1[customer] + cap2[customer] + cap3[customer] + cap4[customer]) > 0;
    }
    ...
    function payable buy(address sender, uint amount)
        payable external whenNotPaused beforeDeadline
        afterStartTime saleNotClosed nonReentrant
    {
        // Update the sender's balance of wei contributed and the amount raised
        uint currentBalance = balanceOf[sender];
        ...
        if (tokenReward.transferFrom(tokenReward.owner(), sender, numTokens)) {
