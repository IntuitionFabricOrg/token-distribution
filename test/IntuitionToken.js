...
contract('IntuitionToken (Basic Tests)', function(accounts) {
  ...
  it("should not allow a regular user to enable transfers", async function() {
      let token = await IntuitionToken.deployed();
      try{
...
contract('IntuitionToken (token burning tests)', function(accounts) {
...
/*
contract('IntuitionToken (Transfer Ownership Tests)', function(accounts) {
  // account[0] points to the owner on the testRPC setup
  var owner = accounts[0];
  var user1 = accounts[1];
  var user2 = accounts[2];
  ...
  it("should be able to transfer all tokens from owner to crowdsale and back", async function() {
      let token = await IntuitionToken.deployed();
      let sale = await IntuitionLaunch.deployed();
