...
contract('Whitelist Crowdsale', function(accounts) {
    ...
    it("should add user2 to the whitelist, deactivate, and reactivate", async function() {
        ...
        var r = await sale.registerUser(user2, {from:owner});
    ...
    */
    ...
    it("should allow user2 to buy tokens up to their limit", async function() {
        ...
        assert.equal(user2Balance + util.oneEther * 5000, user2BalanceAfter1, "token balance of user is incorrect 1");
        await sale.sendTransaction({from: user2,  value: util.oneEther});

        let user2BalanceAfter2 = (await token.balanceOf(user2)).toNumber();

        assert.equal(user2Balance + util.twoEther * 5000, user2BalanceAfter2, "token balance of user is incorrect 2");
    ...
    */
    ...
    it("should allow multiple users to be added to the whitelist", async function() {
        ...
        await sale.registerUsers(addresses, {from:owner});
        ...
        assert.equal(token4, 5000 * util.oneEther, "User4 token balance is wrong");
        assert.equal(token5, 5000 * util.oneEther, "User5 token balance is wrong");
        assert.equal(token6, 5000 * util.twoEther, "User6 token balance is wrong");
