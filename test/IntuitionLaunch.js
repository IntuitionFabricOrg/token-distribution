...
contract('IntuitionLaunch constructor', function(accounts) {
  ...
  it("should be an allowance so that the crowdsale can transfer the tokens", async function() {
      ...
      await token.setTokenLaunch(sale.address, crowdSaleBalance);
...
contract('Multiple Crowdsales', function(accounts) {
  ...
  it("should accept 2 ether for the crowdfunding campaign", async function() {
      ...
      await token.setTokenLaunch(sale.address, crowdSaleAllowance); // ensures crowdsale has allowance of tokens
      ...
      assert.equal(allowance - (amountWei * rate), ownerBalanceAfter, "The crowdsale should have sent amountWei*rate miniAIG");
...
/*
contract('IntuitionLaunch', function(accounts) {
  // account[0] points to the owner on the testRPC setup
  var owner = accounts[0];
  var user1 = accounts[1];
  var user2 = accounts[2];
  ...
  it("should send 2 ether to the crowdfunding campaign", function(done) {
      ...
      IntuitionLaunch.deployed().then(function(instance) {
          sale = instance;
          // return Intuition.send(web3.toWei(amountEther, "ether"));
